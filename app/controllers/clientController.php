	...
	class clientController extends mainModel{
		...
		public function registrarClienteControlador(){
		    ...
		    if($email!=""){
				if(filter_var($email, FILTER_VALIDATE_EMAIL)){
					$sql = $this->conectar()->prepare("SELECT cliente_email FROM cliente WHERE cliente_email = :email");
		    ...
		    $sql = $this->conectar()->prepare("SELECT cliente_id FROM cliente WHERE cliente_tipo_documento = :tipo_documento AND cliente_numero_documento = :numero_documento");
		...
		public function listarClienteControlador($pagina,$registros,$url,$busqueda){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta_datos="SELECT cliente.*, usuario.usuario_nombre as cliente_nombre, usuario.usuario_apellido as cliente_apellido, usuario.usuario_email as cliente_email FROM cliente INNER JOIN usuario ON cliente.usuario_id=usuario.usuario_id WHERE ((cliente.cliente_id!='1') AND (cliente.tipo_documento LIKE :busqueda OR cliente.numero_documento LIKE :busqueda OR usuario.usuario_nombre LIKE :busqueda OR usuario.usuario_apellido LIKE :busqueda OR usuario.usuario_email LIKE :busqueda OR cliente.provincia LIKE :busqueda OR cliente.ciudad LIKE :busqueda)) ORDER BY usuario.usuario_nombre ASC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(cliente.cliente_id) FROM cliente INNER JOIN usuario ON cliente.usuario_id=usuario.usuario_id WHERE ((cliente.cliente_id!='1') AND (cliente.tipo_documento LIKE :busqueda OR cliente.numero_documento LIKE :busqueda OR usuario.usuario_nombre LIKE :busqueda OR usuario.usuario_apellido LIKE :busqueda OR usuario.usuario_email LIKE :busqueda OR cliente.provincia LIKE :busqueda OR cliente.ciudad LIKE :busqueda))";
				...
				$consulta_datos="SELECT cliente.*, usuario.usuario_nombre as cliente_nombre, usuario.usuario_apellido as cliente_apellido, usuario.usuario_email as cliente_email FROM cliente INNER JOIN usuario ON cliente.usuario_id=usuario.usuario_id WHERE cliente.cliente_id!='1' ORDER BY usuario.usuario_nombre ASC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(cliente.cliente_id) FROM cliente WHERE cliente_id!='1'";

			}

			if(!isset($datos) || !isset($total)){
				$stmtDatos = $this->conectar()->prepare($consulta_datos);
				$stmtDatos->execute();
				$datos = $stmtDatos->fetchAll();
				...
				foreach($datos as $rows){
					$tabla.='
						<tr class="has-text-centered" >
							<td>'.$contador.'</td>
							<td>'.$rows['tipo_documento'].': '.$rows['numero_documento'].'</td>
		...
		public function eliminarClienteControlador(){
		    ...
		    # Verificando ventas #
		    $sql = $this->conectar()->prepare("SELECT cliente_id FROM venta WHERE cliente_id = :id LIMIT 1");
		    $sql->bindValue(':id', $id);
		    $sql->execute();
		    $check_ventas = $sql;
