<?php
	...
	private function sanitizeInput($value){
        ...
        public function buscarCodigoVentaControlador(){
			...
			$producto=$this->sanitizeInput($_POST['buscar_codigo'] ?? '');
            ...
            $sql = $this->conectar()->prepare("SELECT producto.*, inventario.stock_actual as producto_stock_total, precio.precio as producto_precio_venta FROM producto INNER JOIN inventario ON producto.producto_id=inventario.producto_id INNER JOIN precio ON producto.producto_id=precio.producto_id AND precio.tipo_precio='minorista' AND precio.cantidad_minima=1 WHERE (producto.nombre LIKE :producto1 OR producto.marca LIKE :producto2 OR producto.modelo LIKE :producto3) ORDER BY producto.nombre ASC");
            $busquedaTerm = '%'.$producto.'%';
            $sql->bindValue(':producto1',$busquedaTerm);
            $sql->bindValue(':producto2',$busquedaTerm);
            $sql->bindValue(':producto3',$busquedaTerm);
            $sql->execute();
            $datos_productos = $sql;
        ...
        public function actualizarProductoCarritoControlador(){
            ...
            $codigo=$this->sanitizeInput($_POST['producto_codigo'] ?? '');
            $cantidad=$this->sanitizeInput($_POST['producto_cantidad'] ?? '');
            ...
            /*== Comprobando producto en la DB ==*/
            $sql = $this->conectar()->prepare("SELECT * FROM producto WHERE producto_codigo=:codigo");
            $sql->bindValue(':codigo',$codigo);
            $sql->execute();
            $check_producto = $sql;
            if($check_producto->rowCount()<=0){
        ...
        public function buscarClienteVentaControlador(){
			...
			$cliente=$this->sanitizeInput($_POST['buscar_cliente'] ?? '');
            ...
            $sql = $this->conectar()->prepare("SELECT * FROM cliente WHERE (cliente_id!='1') AND (cliente_numero_documento LIKE :busqueda OR cliente_nombre LIKE :busqueda2 OR cliente_apellido LIKE :busqueda3 OR cliente_telefono LIKE :busqueda4) ORDER BY cliente_nombre ASC");
            $busquedaTerm = '%'.$cliente.'%';
            $sql->bindValue(':busqueda',$busquedaTerm);
            $sql->bindValue(':busqueda2',$busquedaTerm);
            $sql->bindValue(':busqueda3',$busquedaTerm);
            $sql->bindValue(':busqueda4',$busquedaTerm);
            $sql->execute();
            $datos_cliente = $sql;
        ...
        public function agregarClienteVentaControlador(){
			...
			$id=$this->sanitizeInput($_POST['cliente_id'] ?? '');

			/*== Comprobando cliente en la DB ==*/
			$sql = $this->conectar()->prepare("SELECT * FROM cliente WHERE cliente_id=:id");
			$sql->bindValue(':id',$id);
			$sql->execute();
			$check_cliente = $sql;
        ...
        public function registrarVentaControlador(){
            ...
            $caja=$this->sanitizeInput($_POST['venta_caja'] ?? '');
            $venta_pagado=$this->sanitizeInput($_POST['venta_abono'] ?? '');
            ...
            /*== Comprobando cliente en la DB ==*/
			$sql = $this->conectar()->prepare("SELECT cliente_id FROM cliente WHERE cliente_id=:id");
			$sql->bindValue(':id',$_SESSION['datos_cliente_venta']['cliente_id']);
			$sql->execute();
			$check_cliente = $sql;
			if($check_cliente->rowCount()<=0){
				$alerta=[
            ...
            $sql = $this->conectar()->prepare("SELECT * FROM caja WHERE caja_id=:caja");
            $sql->bindValue(':caja',$caja);
            $sql->execute();
            $check_caja = $sql;
			...
			foreach($_SESSION['datos_producto_venta'] as $productos){
                ...
                $sql = $this->conectar()->prepare("SELECT * FROM producto WHERE producto_id=:id AND producto_codigo=:codigo");
                $sql->bindValue(':id',$productos['producto_id']);
                $sql->bindValue(':codigo',$productos['producto_codigo']);
                $sql->execute();
                $check_producto = $sql;
            ...
            $correlativoStmt = $this->conectar()->query("SELECT COUNT(venta_id) AS total FROM venta");
            $correlativo = ((int) $correlativoStmt->fetchColumn()) + 1;
		...
		public function listarVentaControlador($pagina,$registros,$url,$busqueda){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE (venta.venta_codigo LIKE :busqueda) ORDER BY venta.venta_id DESC LIMIT $inicio,$registros";
				...
				$stmtDatos->bindValue(':busqueda', '%'.$busqueda.'%');
				$stmtDatos->execute();
				...
				$stmtTotal = $this->conectar()->query("SELECT COUNT(venta_id) FROM venta");
				$total = (int) $stmtTotal->fetchColumn();
		...
		public function eliminarVentaControlador(){
			...
			$id=$this->sanitizeInput($_POST['venta_id'] ?? '');

			# Verificando venta #
		    $sql = $this->conectar()->prepare("SELECT * FROM venta WHERE venta_id=:id");
	    $sql->bindValue(':id',$id);
	    $sql->execute();
	    $datos = $sql;
		    if($datos->rowCount()<=0){
		    ...
		    $sql = $this->conectar()->prepare("SELECT venta_detalle_id FROM venta_detalle WHERE venta_codigo=:codigo");
	    $sql->bindValue(':codigo',$datos['venta_codigo']);
	    $sql->execute();
	    $check_detalle_venta = $sql;
