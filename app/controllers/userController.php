<?php
	...
	class userController extends mainModel{
		...
		public function registrarUsuarioControlador(){
            ...
            # Verificando usuario #
		    $sql = $this->conectar()->prepare("SELECT usuario_usuario FROM usuario WHERE usuario_usuario=:usuario");
		    ...
		    # Verificando caja #
		    $sql = $this->conectar()->prepare("SELECT caja_id FROM caja WHERE caja_id=:caja");
		...
		public function listarUsuarioControlador($pagina,$registros,$url,$busqueda){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta_datos="SELECT * FROM usuario WHERE ((usuario_id!=:id AND usuario_id!=1) AND (usuario_nombre LIKE :busqueda OR usuario_apellido LIKE :busqueda2 OR usuario_email LIKE :busqueda3 OR usuario_usuario LIKE :busqueda4)) ORDER BY usuario_nombre ASC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(usuario_id) FROM usuario WHERE ((usuario_id!=:id AND usuario_id!=1) AND (usuario_nombre LIKE :busqueda OR usuario_apellido LIKE :busqueda2 OR usuario_email LIKE :busqueda3 OR usuario_usuario LIKE :busqueda4))";

				$stmtDatos = $this->conectar()->prepare($consulta_datos);
				$term = '%'.$busqueda.'%';
				$stmtDatos->bindValue(':id',$_SESSION['id']);
				$stmtDatos->bindValue(':busqueda',$term);
				$stmtDatos->bindValue(':busqueda2',$term);
				$stmtDatos->bindValue(':busqueda3',$term);
				$stmtDatos->bindValue(':busqueda4',$term);
				$stmtDatos->execute();
				$datos = $stmtDatos->fetchAll();

				$stmtTotal = $this->conectar()->prepare($consulta_total);
				$stmtTotal->bindValue(':id',$_SESSION['id']);
				$stmtTotal->bindValue(':busqueda',$term);
				$stmtTotal->bindValue(':busqueda2',$term);
				$stmtTotal->bindValue(':busqueda3',$term);
				$stmtTotal->bindValue(':busqueda4',$term);
				$stmtTotal->execute();
				$total = (int) $stmtTotal->fetchColumn();

			}else{

				$consulta_datos="SELECT * FROM usuario WHERE usuario_id!=:id AND usuario_id!=1 ORDER BY usuario_nombre ASC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(usuario_id) FROM usuario WHERE usuario_id!=:id AND usuario_id!=1";
		...
		public function eliminarUsuarioControlador(){
		    ...
		    # Verificando ventas #
		    $sql = $this->conectar()->prepare("SELECT usuario_id FROM venta WHERE usuario_id=:id LIMIT 1");
		    $sql->bindValue(':id',$id);
		    $sql->execute();
		    $check_ventas = $sql;
		...
		public function actualizarUsuarioControlador(){
		    ...
		    $sql = $this->conectar()->prepare("SELECT * FROM usuario WHERE usuario_usuario=:usuario AND usuario_id=:id");
		    $sql->bindValue(':usuario',$admin_usuario);
