<?php
	...
	class mainModel{
		...
		/*---------- Funcion seleccionar producto completo ----------*/
		public function seleccionarProductoCompleto($id){
			$id=$this->limpiarCadena($id);
			...
			$sql=$this->conectar()->prepare("SELECT 
			  ...
			  COALESCE(i.stock_actual, 0) as producto_stock_total,
			  COALESCE(pc.precio, 0) as producto_precio_venta,
			  COALESCE(p.precio_base, 0) as producto_precio_compra,
			  p.tipo as producto_tipo_unidad,
			  c.nombre as categoria_nombre
			FROM producto p
			LEFT JOIN inventario i ON p.producto_id = i.producto_id
			LEFT JOIN precio pc ON p.producto_id = pc.producto_id AND pc.tipo_precio = 'minorista' AND pc.cantidad_minima = 1
