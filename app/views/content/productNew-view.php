...
<div class="container pb-6 pt-6">
	...
	<form class="FormularioAjax" action="<?php echo APP_URL; ?>app/ajax/productoAjax.php" method="POST" autocomplete="off" enctype="multipart/form-data" >
		...
		<div class="columns">
			<div class="column">
				...
				<p class="has-text-centered mb-4">Puedes pegar una imagen directamente con Ctrl+V</p>
				...
				<div id="image-preview" class="has-text-centered mb-6"></div>
			</div>
		</div>
	...
	<script>
		document.addEventListener('paste', function(e) {
			...
			for (var i = 0; i < items.length; i++) {
				if (items[i].type.indexOf('image') !== -1) {
					...
					var fileInput = document.querySelector('input[name="producto_foto"]');
		...
		document.querySelector('input[name="producto_foto"]').addEventListener('change', function(e) {
			var file = e.target.files[0];
			if (file && file.type.startsWith('image/')) {
				var reader = new FileReader();
				reader.onload = function(e) {
					document.getElementById('image-preview').innerHTML = '<img src="' + e.target.result + '" style="max-width: 200px; max-height: 200px;">';
				};
